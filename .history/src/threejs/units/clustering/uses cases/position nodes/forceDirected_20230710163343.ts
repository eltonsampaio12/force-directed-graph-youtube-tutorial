...
function calculateRepulsiveForce(
  ...
  const disconnectedNodes = Graph.getInstance().nodes.filter((item) => {
    ...
    return neighbors !== undefined && item != node && !neighbors.includes(item);
  });
    ...
    if (node !== child) {
      ...
      if (distance !== 0) {
        if (distance < repulsionDistance) {
          ...
          node.force.add(repulsiveForce.multiplyScalar(4));
        }
      }
    }
  });
  ...
  node.getNeighbors().forEach((child) => {
    if (node !== child) {
      ...
      if (distance !== 0) {
        ...
        node.force.sub(attractiveForce.multiplyScalar(4));
        child.force.add(attractiveForce.multiplyScalar(4));
      }
    }
  });
}
