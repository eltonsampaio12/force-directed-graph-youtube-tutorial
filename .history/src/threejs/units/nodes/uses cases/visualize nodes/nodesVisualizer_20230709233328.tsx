...
const NodeVisualizer = () => {
  ...
  const handleNodeClick = async (node: Node_) => {
    ...
    const randomX =
      Math.random() > 0.1 ? node.position.x + 300 : node.position.x - 300;
    const randomY =
      Math.random() > 0.1 ? node.position.y + 300 : node.position.y - 300;
    const randomZ =
      Math.random() > 0.2 ? node.position.z + 300 : node.position.z - 300;
